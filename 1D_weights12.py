# Code for automated computations for different parameters within weights 1 and 2
...
def weight1(argument, a_const = 200, b_const = 4000):
...
for i in range(number_for_averaging):
    ...
    for i in range(1, number_for_comparison):
        ...
        a_weight = current_weight(argument=grad_norm, a_const = number_for_scaling * i, b_const = number_for_scaling * number_for_ratio * i)
...
for i in range(1, number_for_comparison):
    ...
    print(f"Modified ROF, a = {number_for_scaling * i}, b = {number_for_scaling * number_for_ratio * i} (acc) - average TV error: {average_mROF[i]}")

    print(f"Modified ROF, a = {number_for_scaling * i}, b = {number_for_scaling * number_for_ratio * i} (acc) - average l2 distance (image): {average_l2distance_mROF[i]}")

    print(f"Modified ROF, a = {number_for_scaling * i}, b = {number_for_scaling * number_for_ratio * i} (acc) - average l2 distance (noisy): {average_l2distance_noisy_mROF[i]}")

    print(f"Modified ROF, a = {number_for_scaling * i}, b = {number_for_scaling * number_for_ratio * i} (acc) - number of iterations: {average_iterations_mROF[i]}")
...
weight_values = [number_for_scaling * i for i in range(1, number_for_comparison)]
plt.plot(weight_values, average_ROF_acc_plot[1:], label="ROF")
plt.plot(weight_values, average_mROF[1:], label=f"dpROF, b/a = {number_for_ratio}")

plt.xlabel("Values of a")
plt.ylabel("Average TV Error")
#plt.title(f"Weight {weight_labels[current_weight]}, σ = {current_noise}, f = {test_function_labels[current_test_function]}")
...
plt.plot(weight_values, average_l2distance_mROF[1:], label=f"dpROF, b/a = {number_for_ratio}")
plt.xlabel("Values of a")
