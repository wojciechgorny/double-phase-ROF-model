# Code for automated computations for different radii in mollification
...
# Number of radii to test (the highest value in pixels)

number_for_comparison = 21
...
for i in range(number_for_averaging):
    ...
    # Step 2: Run the accelerated double-phase adaptive ROF, radius zero (no mollification)

    # First generate the weight from the accelerated classical ROF

    mollified_v1_acc = denoised_v1_acc.copy()
    ...
    technical_sum_mROF[0] += grad_norm
    l2distance_sum_mROF[0] += l2distance
    l2distance_noisy_sum_mROF[0] += l2distance_noisy
    iterations_sum_mROF[0] += iter_v2_acc
    ...
    for i in range(1, number_for_comparison):
        ...
        # Step i+2: Run the accelerated double-phase adaptive ROF, radius i

        # First generate the weight from the accelerated classical ROF

        mollified_v1_acc = mollify_function(denoised_v1_acc, radius=i)
        grad_v1 = gradient(mollified_v1_acc)
        grad_norm = norm2(grad_v1)
...
for i in range(0, number_for_comparison):
    ...
    print(f"Modified ROF, radius {i} (acc) - average TV error: {average_mROF[i]}")

    print(f"Modified ROF, radius {i} (acc) - average l2 distance (image): {average_l2distance_mROF[i]}")

    print(f"Modified ROF, radius {i} (acc) - average l2 distance (noisy): {average_l2distance_noisy_mROF[i]}")

    print(f"Modified ROF, radius {i} (acc) - number of iterations: {average_iterations_mROF[i]}")
...
plt.plot(average_ROF_acc_plot, label="ROF")
plt.plot(average_mROF, label="dpROF")

plt.xlabel("The mollification radius")
